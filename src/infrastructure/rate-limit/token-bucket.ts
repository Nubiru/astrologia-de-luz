...
/**
 * RateLimitGate adapter (S-2 §7.2.4 A). Wraps `checkRateLimit` to satisfy
 * the domain port shape — the port intentionally omits `hourBucket` (telemetry
 * only); the wrapper drops it. Authored W4-3a per spec §7.2.5 composition
 * import `makeRateLimitGate`.
 */
export function makeRateLimitGate<TSchema extends Record<string, unknown>>(
  db: LibSQLDatabase<TSchema>,
): RateLimitGate {
  return {
    async check(ip: string) {
      const { allowed, retryAfterSeconds, count } = await checkRateLimit(db, ip);
