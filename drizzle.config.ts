 ...
 * `db/migrations/meta/` snapshot only) — they are required by
 * `drizzle-kit push` / `studio` if either is ever wired in. The
 ...
 * `0003_seed_augusto` already authored AND the `db/migrations/meta/`
 * snapshot in lockstep, a second `drizzle-kit generate` must emit
 * `No schema changes, nothing to migrate` and create no new `.sql` file.
 * The regression-guard lives in `tests/ci/drizzle-generate-diff.spec.ts`.
 *
