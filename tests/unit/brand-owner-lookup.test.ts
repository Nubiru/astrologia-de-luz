/**
 * G_C-8 unit pairing — brand-owner identification (§11 intro + AC-3.2.1).
 *
 * What this catches:
 ...
 *   - `getBrandOwner()` does not constrain by the canonical email → any
 *     teachers row with a similar-but-not-equal email matches.
 ...
 * SQL the helper emits is exercised end-to-end (not a mocked client).
 */
...
import { brandOwnerEmail, getBrandOwner } from '@/lib/brand-owner';
...
describe('getBrandOwner — DB lookup (§11 intro + AC-3.2.1)', () => {
  test('returns the Teacher row when the seeded brand-owner exists', async () => {
    ...
    const owner = await getBrandOwner(db, 'augusto@astrologiadeluz.com');
  ...
  test('picks the right row among multiple teachers (matches FIRST ADMIN_EMAILS entry only)', async () => {
    ...
    const owner = await getBrandOwner(db, 'augusto@astrologiadeluz.com,backup@example.com');
  ...
  test('flipping ADMIN_EMAILS[0] resolves to a DIFFERENT row', async () => {
    ...
    const augusto = await getBrandOwner(db, 'augusto@astrologiadeluz.com');
    const maria = await getBrandOwner(db, 'maria@astrologiadeluz.com');
